...
public class Transformer {
   ...
   */
  private static final float FILT0 = -0.04739361256361008F;
  private static final float FILT1 = -0.04662604257464409F;
  private static final float FILT2 = -0.00119693996384740F;
  private static final float FILT3 = -0.03382463753223419F;
  private static final float FILT4 =  0.02838252671062946F;
  private static final float FILT5 = -0.01656247489154339F;
  private static final float FILT6 =  0.07613456249237061F;
  private static final float FILT7 =  0.07250666618347168F;
  private static final float FILT8 = -0.03595214337110519F;
  private static final float FILT9 = -0.08879133313894272F;
  ...
  private void lotRev8( float[] x, int index, int nblocks, float[] scratch ) {
    ...
    for ( i=0; i<nblocks; i++ ) {
       ...
       * non-zero. */
      _tmp[0] = FILT0 * x[xIndex];   _tmp[1] = FILT8 * x[xIndex];
      ...
      if ( x[2+xIndex] != 0.0 ) {
        _tmp[0] += FILT2 * x[2+xIndex];   _tmp[1] += FILT10 * x[2+xIndex];
      ...
      if ( x[4+xIndex] != 0.0 ) {
        _tmp[0] += FILT4 * x[4+xIndex];   _tmp[1] += FILT12 * x[4+xIndex];
      ...
      if ( x[6+xIndex] != 0.0 ) {
        _tmp[0] += FILT6 * x[6+xIndex];   _tmp[1] += FILT14 * x[6+xIndex];
      ...
      _tmp[8] = FILT1 * x[1+xIndex];    _tmp[9] = FILT9 * x[1+xIndex];
      ...
      if ( x[3+xIndex] != 0.0 ) {
        _tmp[8] += FILT3 * x[3+xIndex];    _tmp[9] += FILT11 * x[3+xIndex];
      ...
      if ( x[5+xIndex] != 0.0 ) {
        _tmp[8] += FILT5 * x[5+xIndex];    _tmp[9] += FILT13 * x[5+xIndex];
      ...
      if ( x[7+xIndex] != 0.0 ) {
        _tmp[8] += FILT7 * x[7+xIndex];    _tmp[9] += FILT15 * x[7+xIndex];
   ...
   * @param  scratch  work array - must be nsamps+32 in length.
   */
  /*package*/ void lotFwd(float[] x, int index, int blockSize, int transLength,
                          int nblocks, float[] scratch) {
    ...
    if (transLength == 8) {
      ...
      if (blockSize > 8)
        for (i=0; i<nblocks; i++) multiplex8(x, i*blockSize+index, nsubBlocks, scratch);
    } else if (transLength == 16) {
      nsubBlocks = blockSize / 16;
      assert nsubBlocks*16 == blockSize;
      lotFwd16(x, index, nsubBlocks*nblocks, scratch);
      if (blockSize > 16)
        for (i=0; i<nblocks; i++) multiplex16(x, i*blockSize+index, nsubBlocks, scratch);
    } else {
      assert false;
    }
  }
   ...
   * @param  scratch  work array - must be nsamps+32 in length.
   */
  /*package*/ void lotRev(float[] x, int index, int blockSize, int transLength,
                          int nblocks, float[] scratch) {
